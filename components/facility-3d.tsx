...
// Update the SimulationState interface to match the new structure used in the main component:
...
export function Facility3D({ simulation }: Facility3DProps) {
  ...
  const equipmentData = useMemo(() => {
    // Update the equipment status logic to use the new heatwave properties:
    const getServerStatus = (id: string): EquipmentStatus => {
      ...
      if (simulation.heatwaveActive) {
        if (simulation.heatwaveIntensity === "extreme") {
          temperature = 45
          if (id === "R01" || id === "R02") status = "critical"
        } else if (simulation.heatwaveIntensity === "severe") {
          temperature = 38
          if (id === "R01" || id === "R02") status = "warning"
        } else if (simulation.heatwaveIntensity === "moderate") {
          temperature = 32
        }
      }
    ...
    const getPowerStatus = (id: string): EquipmentStatus => {
      ...
      if (simulation.operationMode === "islanded" && id === "PDU01") {
        status = "offline"
        load = 0
      } else if (simulation.heatwaveActive && simulation.heatwaveIntensity === "severe") {
        load = 85
        status = "warning"
      }
